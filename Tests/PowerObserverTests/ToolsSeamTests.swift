...
/// `power-observer`'s brief is explicit about what it must never do: veto a sleep it was
/// only asked whether it could, touch the SMC by any route, or import the privileged
/// helper. `ToolsSeamScanner` is what checks that against the source rather than against a
/// reader's memory of it — the same argument `Tests/AeolusHelperTests/SeamScanner.swift`'s
/// suites make for `Sources/AeolusHelper`, applied to `Tools/PowerObserver` with a scanner
/// of its own. See `ToolsSeamScanner`'s header note for why the two do not share an
/// implementation, and for why this suite is scoped to `Tools/PowerObserver` rather than
/// all of `Tools/`.
@Suite("Tools/PowerObserver forbidden tokens")
struct ToolsSeamTests {
    ...
    @Test("no forbidden token appears under Tools/PowerObserver", arguments: forbiddenTokens)
    func toolsNeverNamesAForbiddenToken(_ token: String) throws {
        #expect(
            try !ToolsSeamScanner.anyFileContains(token),
            "Tools/PowerObserver must never name \(token) — see Tools/PowerObserver's brief.")
    }
    ...
    /// `Tools/PowerObserver` currently contains.
