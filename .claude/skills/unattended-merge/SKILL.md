---
description: "Defines when an autonomous session (e.g. `/autonomous-dev-flow`, `/tackle-issues`) may merge its **own** PR without a human pause, and what record it must le..."
---
...
```bash
gh pr merge ${PR_NUM} --squash --delete-branch   # squash merge — matches /prime-directive's "synchronous squash merge" rule and this repo's plain-imperative, one-PR-one-commit convention on main
# Verify — do not trust the exit code alone:
gh pr view ${PR_NUM} --json state --jq .state    # must print MERGED
```

After a verified merge, run any repo post-merge steps. `None — Aeolus has no automated release-tag update or deploy hook tied to a merge into main; the only thing that runs afterward is the same required CI on whatever branch is up next.`
