---
description: "Address PR review findings from inline threads and general review summaries, verify their disposition, and respond where each was raised."
---
...
# Add safety-critical for helper, SMC write-path, XPC-boundary or docs/SAFETY.md findings.
# Required checks: Build and test, Monitor app build, Lint and format.
# Run swift build && swift test before pushing a code fix; format with
# swift format format --in-place --recursive Sources Tests when appropriate.
# Never weaken a safety check to resolve a privileged-code finding.
