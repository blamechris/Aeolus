...
1. **Detect the current version** by reading `project.yml` (the `MARKETING_VERSION` setting), cross-checked against `CHANGELOG.md``.
...
4. **On confirmation**:
   ...
   - Bump version across all version-bearing files: `project.yml` (`MARKETING_VERSION`, and `CURRENT_PROJECT_VERSION` for the build number), the `version:` string in `Sources/fanctl/FanctlCommand.swift`, and a new `## [Unreleased]` heading in `CHANGELOG.md`
   - Run `grep -n "MARKETING_VERSION\|CURRENT_PROJECT_VERSION" project.yml && grep -n "version:" Sources/fanctl/FanctlCommand.swift` and confirm each file shows the new version.
...
**Labels:** Always tag with one severity (`bug` / `enhancement` / `ux`) plus one or more surface labels: `area:smc`, `area:helper`, `area:ui`, `area:cli`, `area:catalog`, `area:ci`.

Add `safety-critical` to anything where fans failed to return to automatic, a fan reached 0 RPM, the app reported a speed the hardware was not honouring, or the machine overheated while Aeolus held control. Those are triaged ahead of everything else. Add `needs-hardware` when the finding cannot be reproduced in CI — which is most fan-control findings, since GitHub's macOS runners are VMs with no SMC.

**Hardware reality:** testing happens on a single `Mac16,5` (Apple M4 Max, macOS 26.5.2). That is an M3-or-newer part, so the `Ftst` unlock path is exercisable, but the Intel and M1/M2 paths are not testable at all — never file a finding as fixed on those, and never move a `docs/HARDWARE-MATRIX.md` row off `untested` from this machine.

Always exercise these before calling a fan-control change good: fans return to automatic on quit **and** on `kill -9`; manual control survives sleep and wake; actual RPM follows target and reclamation is reported rather than hidden; `fanctl reset --all` works from a cold start with the app not running.
...
- It does not run automated UI tests or smoke checks (Aeolus has no automated smoke-test skill — hardware behaviour cannot be verified in CI, which is precisely why this manual mode exists).
...
<!-- skill-templates: manual-testing-mode a1368f4 2026-07-25 -->
