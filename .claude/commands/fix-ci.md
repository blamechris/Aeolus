...
# Aeolus workflows: "CI" carries the three required checks (Build and test,
# Monitor app build, Lint and format). "Validate sensor catalog" runs only on
# changes under Resources/catalog/.
...
```bash
# macOS runners are slow to start and the Monitor app build resolves SwiftPM
# dependencies and runs xcodebuild. Budget for that rather than the default.
...
Aeolus-specific patterns:
- `Configs/Signing.xcconfig` missing, or any code-signing error in **Monitor app build** → **FIX, never work around.** That file is gitignored and absent on CI and on every contributor's machine. If the build now needs it, the repository has grown a dependency on the maintainer's certificate and locked out every outside contributor. Never add signing material to CI to make this pass.
- `swift format lint` failures → FIX with `swift format format --in-place --recursive Sources Tests`.
- SwiftLint `no_zero_rpm_literal` / `no_unchecked_sendable_in_helper` / `no_silent_write_failure` → **FIX the code.** These are safety invariants from `docs/SAFETY.md` expressed as lint rules. Never add an inline `swiftlint:disable` to clear one.
- A test failing because it needs real hardware → the test does not belong in CI. GitHub's macOS runners are VMs with no SMC and cannot verify a fan write. Move it to the manual hardware checklist.
- `xcodegen` spec validation errors → FIX `project.yml`; never commit the generated `Aeolus.xcodeproj`.
...
# Fallback: push an empty commit to the PR branch to force a fresh run.
...
```bash
# Same budget as above — macOS runners plus xcodebuild.
MAX_WAIT=1500
INTERVAL=30
ELAPSED=0
...
<!-- skill-templates: fix-ci 3768ea6 2026-07-25 -->
