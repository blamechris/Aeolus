...
Aeolus has no separate deploy workflow — `ci.yml` (build + test) and `catalog-validate.yml` (hardware catalog checks) are the only workflows, and neither represents a release/deploy step (a `Full` signed build is produced manually by the maintainer, never CI, since CI runners have no SMC to verify against). Skip the deploy-health query entirely.

Summarize: what shipped recently and whether the latest default-branch CI run (both `ci.yml` and `catalog-validate.yml`) is green. Call out any failing run on the default branch explicitly — that's the "something's on fire" signal.
...
Note when several stale PRs predate a recent toolchain/dependency change on the default branch — their failures may simply be a missing rebase. Flag any PR touching `Sources/AeolusHelper` or `Sources/SMCCore`'s write path explicitly — those need Opus review per CLAUDE.md regardless of CI state.
...
Group by Aeolus's real label families: `epic` (E0–E15), `milestone:M0`–`M4`, `area:smc` / `area:helper` / `area:ui` / `area:cli` / `area:catalog` / `area:ci`, plus flags `safety-critical`, `needs-hardware`, `good-first-issue`, `no-signing-required`, `hardware-report`. Surface `safety-critical` and `needs-hardware` items prominently — they can't be picked up in a CI-only session.

Surface the count, the most recently updated items, and any `bug`/`safety-critical`/`blocked` issues at the top. Do not read every issue body — this is a recap, not triage (`/start-working` does the deep prioritization).
...
Report the current branch, ahead/behind vs upstream, count of uncommitted/staged files, and any stashes — these are the "you were mid-something" signals. Note that `Aeolus.xcodeproj` and `Configs/Signing.xcconfig` are gitignored by design; don't flag them as unexpected dirty state.
...
### Recent activity
{1-3 lines: what shipped, latest CI state — green or red}
...
### Open issues ({N})
{grouped by label theme, 1 line per theme; flag any bug/safety-critical/blocked at top}
...
<!-- skill-templates: catchup 3f7f775 2026-07-30 -->
