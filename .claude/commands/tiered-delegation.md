...
`fable > opus > sonnet > haiku`. Subagent definitions in `.claude/agents/` pin each tier: `architect` (fable), `reviewer` (opus), `implementer` (sonnet), `chore-runner` (haiku).

Route by **blast radius, not apparent difficulty** — a three-line change in `Sources/AeolusHelper` outranks a thousand-line SwiftUI refactor. This project ships a root daemon that writes to firmware, so the cost of a cheap tier getting it wrong is not a failed test.

**Hard rule:** no Sonnet-authored code lands in `AeolusHelper` or `SMCCore`'s write path without Opus review. Do not assume `fable` is present in every harness; fall back to `opus` and say so rather than silently downgrading a decision that was meant to be escalated.
...
- Every worker brief is **self-contained**: repo context, exact scope, house style, what NOT to touch, the return format, and the project's exploration shortcut (read `CLAUDE.md` and `docs/ARCHITECTURE.md` first — they carry the safety invariants and the target map. Subagents do not inherit `CLAUDE.md`, so restate the relevant hard rules in the brief itself, especially the ones about clamping, the lease, and never claiming unverified hardware support).
...
<!-- skill-templates: tiered-delegation 01903fb 2026-07-25 -->
