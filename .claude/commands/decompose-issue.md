...
**Labels each sub-issue will receive:** enhancement, parent:#${PARENT_NUM}, the parent's `area:*` label and milestone, plus `safety-critical` if the parent carries it and `needs-hardware` for anything CI cannot verify
...
```bash
# Aeolus uses no complexity labels. Sub-issues inherit the parent epic's area
# and milestone labels, because a child of an area:helper epic is still helper work.
SUB_LABELS="enhancement,${PARENT_AREA_LABEL},${PARENT_MILESTONE_LABEL}"

# A child of a safety-critical epic is itself safety-critical. Never drop this
# label during decomposition — it is what routes the issue to Opus review.
if [ "$PARENT_IS_SAFETY_CRITICAL" = "true" ]; then
  SUB_LABELS="$SUB_LABELS,safety-critical"
fi

# Always tag with parent reference (if the repo uses such a label; skip if not)
# Aeolus has no parent:#N label. The body's "Part of #N" line is the link.
#
# Sequencing rule: never produce a sub-issue that lets a write path merge before
# E5, the safety subsystem. If a child writes to the SMC, its body must say
# "Blocked by E5" and the epic dependency must survive decomposition.
...
```bash
# Aeolus uses no "decomposed" marker label — parent epics already carry `epic`,
# and their checklist shows decomposition status directly. Skip this step.
```
...
<!-- skill-templates: decompose-issue b4268c2 2026-07-25 -->
