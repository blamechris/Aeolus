...
Then state, in one short block: branch/HEAD, dirty files, open PRs (mine/others), drifted skills, the delegated outcome and observable acceptance, what is usable now, and the next gap this session will close. Reuse the user's existing authorization; a seed is context, not a request for another scope approval.
...
### The interactive flow — use the authority already granted

An operator watching does not itself require a pause. Resolve routine implementation choices and verified adequate fallbacks autonomously; preserve genuine owner-reserved decisions and repository gates.

1. **Scope, at the start.** State the outcome and bounded next increment. Ask only for missing authority or an indispensable preference; do not repeat an approval already given.
2. **Merge, when ready.** Follow the repo's `CLAUDE.md` gate and actual merge authority. If a human merge approval is required and has not been given, prepare the concrete reviewed result before asking. If self-merge is authorized, satisfy its gates and proceed.

**For already-authorized steps, act.** Specifically, never ask:

- *"want a PR?"* — PR-first is already the workflow; open it.
- *"merge or review?"* — review is mandatory; run it, never offer to skip it.
- *"shall I fix the findings?"* — fix them and re-run the gates.

Ask only when a needed decision is outside delegated authority or cannot be resolved from available evidence. Put that precise decision in the `DECISION` slot and continue independent authorized work. An authorized fallback verified to preserve safety, correctness, required runtime/cost constraints and essential capability needs no owner pause: record the evidence, file the underlying problem and continue. If it cannot be verified, block only the affected item with the reason.

**Unattended sessions use the same review and safety gates.** Delegation does not grant missing signing, hardware, deployment or merge authority. A genuine reservation blocks its dependent work; it does not stop all other work toward the outcome.

**Clearing the session** follows the global "Session boundaries" rule. Preserve every PR's verified state (merged on `origin/main`, or safely retained open with its remaining gate), account for in-flight work and follow-ons, and retain the outcome, acceptance and run limits in the seed. When authorized work remains, end for a restart only after an orchestrator has accepted the next run; record its identifier. Otherwise use supported host continuation/compaction. If neither can continue, report the host limitation and exact restart action. A second compaction or work-class switch is a context-management trigger, not product completion or proof that a user decision is needed.
...
1. **Handoff seed (artifact ②)** — **the seed is written outside every worktree, at an absolute path:** `$CLAUDE_HANDOFF_DIR/NEXT-<scope>.md`, default dir `~/Obsidian/no-it-all/handoffs/`. It holds what shipped, what remains, and *why* each remaining item is blocked or deferred. Not in the repo, not in this session's worktree, not on a branch — an absolute path outside every *session's* workspace has **no worktree to be torn down with, no branch to be unreachable from, and no index to be confused with**, which is why nothing downstream has to be gated on it. Three earlier attempts to keep the seed inside a session's own workspace failed on exactly those surfaces, the last of them on a "committed" proof that had only staged the file. Read the property precisely: the default handoff dir is itself a git repo (the vault is versioned, and should be), and that takes nothing away — what the seed must be outside is the tree *this session* edits, branches and tears down, which is exactly what the write checks it against.
   ...
   **One command writes it.** It resolves the scope, resolves this session's id, archives whatever it collides with, writes the seed, and proves the result — all of it in `session-seed.py`, none of it transcribed here:
   ...
   Fill the body with the **delegated outcome and observable acceptance**, **what is usable today** (build/install and evidence, or unknown), **the remaining gap**, and **the actual execution state**: continuing/accepted next run, ready for user QA, budget/retry limit, specific blocker, host continuation limit, or complete. Preserve the authority and consumed run limits. Name a user action only when genuinely needed, with what it unblocks. The script writes and verifies the file; it does not infer these answers or schedule continuation.
...
6. **Cleanup** — remove this session's worktrees (leave other sessions' worktrees alone), prune branches only after verifying each had a `MERGED` PR, stop dev daemons/servers started this session, restore any test-env mutations (if a test build of `AeolusHelper` was manually loaded via `launchctl` during this session, unload it and confirm with `launchctl print`; do not leave a stray root daemon. Never leave `Configs/Signing.xcconfig` or a Team ID staged).
...
7. **Closing message** — lead with the outcome, usable state and remaining gap from the seed. Include the verified absolute seed path (`seed:` from step 1) and any open PR number. End with the global `**Status:**` block and `**Next:**` line naming actual execution: accepted next task/session, continuing work, genuine QA/access/decision, a reached limit, or complete. The stable seed path is the one line to paste if a manual restart is truly required; its existence alone is never evidence that work will continue.
...
<!-- skill-templates: session-lifecycle 193a3f5 2026-09-23 -->
