...
# Copilot review presence — blamechris/Aeolus is on GitHub.com, so the bot
# login is copilot-pull-request-reviewer[bot]. Copilot review here is
# best-effort (may be slow or absent); Step 2b below already treats an
# absent review on an old PR as a warning rather than a hard stop.
...
```bash
# Aeolus's three required checks on main (.github/workflows/ci.yml).
# "Validate sensor catalog" is deliberately excluded — it only runs on
# catalog changes, so requiring it here would block merges that never
# trigger it.
REQUIRED_CHECKS=("Build and test" "Monitor app build" "Lint and format")
...
# Same bot login as Phase 1 — copilot-pull-request-reviewer[bot] on GitHub.com.
...
if [ "$DRY_RUN" = true ]; then
  ...
  # Squash merge, matching /prime-directive's "synchronous squash merge" rule
  # and this repo's one-PR-one-commit convention on main.
...
```bash
# Same budget /fix-ci uses for this repo's CI: macOS runners are slow to
# start and the Monitor app build resolves SwiftPM dependencies and runs
# xcodebuild, so a 3-minute window is too tight.
...
<!-- skill-templates: batch-merge 8c8e472 2026-07-27 -->
