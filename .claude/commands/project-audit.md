...
| Privilege Boundary | "Warden" | The `AeolusHelper` root daemon: XPC client code-signing checks, entitlements, `SMAppService` lifecycle, lease expiry on crash/`SIGKILL`, and whether `SMCCore`'s write API stays `package`-scoped | `Sources/AeolusHelper` or `Sources/AeolusXPC` exist | Root-privilege reviewer who assumes the daemon will be attacked or will crash mid-write. Checks that every path back to automatic fan control is reachable, that no client is trusted merely for connecting, and that nothing `SMCCore` exposes as `package` has quietly gone `public`. |
...
For Aeolus, Sentinel should weight the privilege boundary above generic web-app concerns: client code-signing verification in `AeolusXPC`, entitlements, and whether `SMCCore`'s write API stays `package`-scoped outweigh dependency CVEs. Inspector should weight whether hardware-dependent tests skip cleanly on CI (GitHub's macOS runners have no SMC) rather than penalizing untestable hardware paths as "no coverage" — a fan-write test that fails on a VM is a bug in the test, not evidence of an untested feature.
...
<!-- skill-templates: project-audit aae4d65 2026-07-27 -->
