...
For Aeolus, weight two of the core lenses toward this repo's actual risk rather than a typical web app's: **Sentinel** (Security) should treat `Sources/AeolusHelper`, the SPI-gated write path in `Sources/SMCCore`, `Sources/AeolusXPC`, and `Configs/*.entitlements` as the highest-scrutiny surface — this is a root launch daemon writing to firmware, not a web attack surface, so client code-signing checks and the SMC write boundary matter more than injection/XSS classes that don't apply here. **Strategist** (Feature Completeness) should read "gaps" against `docs/SAFETY.md` and the epic board rather than generic UX polish — a missing safety check (lease expiry, firmware-bound clamping) is a gap of a different order than a missing button.
...
| Privilege Boundary | "Warden" | The `AeolusHelper` root daemon: XPC client code-signing checks, entitlements, `SMAppService` lifecycle, lease expiry on crash/`SIGKILL`, and whether `SMCCore`'s write API stays behind the `FanWrite` SPI group | `Sources/AeolusHelper` or `Sources/AeolusXPC` exist | Root-privilege reviewer who assumes the daemon will be attacked or will crash mid-write. Checks that every path back to automatic fan control is reachable, that no client is trusted merely for connecting, and that nothing `SMCCore` exposes through `@_spi(FanWrite)` has quietly gone plain `public`, and that the group still has exactly two members. |
...
Aeolus titles are plain imperative subjects, never conventional-commit prefixes (`type(scope):`) — matching `/create-issue`'s convention in this repo. Apply the matching `area:*` label (`area:smc`, `area:helper`, `area:ui`, `area:cli`, `area:catalog`, `area:ci`) when a finding maps to one, plus `safety-critical` for anything touching `Sources/AeolusHelper`, the `SMCCore` write path, `Sources/AeolusXPC`, or a mechanism in `docs/SAFETY.md`, and `needs-hardware` for anything that cannot be verified on GitHub's macOS runners.
...
For Aeolus, Sentinel should weight the privilege boundary above generic web-app concerns: client code-signing verification in `AeolusXPC`, entitlements, and whether `SMCCore`'s write API stays behind the `FanWrite` SPI group outweigh dependency CVEs. Inspector should weight whether hardware-dependent tests skip cleanly on CI (GitHub's macOS runners have no SMC) rather than penalizing untestable hardware paths as "no coverage" — a fan-write test that fails on a VM is a bug in the test, not evidence of an untested feature.
...
<!-- skill-templates: project-audit 8196307 2026-07-30 -->
