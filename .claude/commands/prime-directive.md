...
3. **Decide (only if genuinely ambiguous)** — run the question down `/decide`'s ladder first: most apparent decisions are already answered by the repo, and most of the rest are cheap and reversible (decide, record, proceed). For what survives that — a real decision (epic scope, design fork, choosing among N approaches) — run the decision panel (the `architect` sub-agent), **pick the recommended option**, and **record the decision** in the session log plus a one-line note on the issue. Never block on the user.
...
<!-- skill-templates: prime-directive d318c59 2026-07-27 -->
